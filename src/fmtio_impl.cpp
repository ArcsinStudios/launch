...
namespace launch {
	void fmtout(const std::string& fmt, const hedgehog& captures, std::ostream& out) {
		...
		for (size_t i = 0; i < fmt_length; ++i) {
			...
			case '{':
				if (i + 1 == fmt_length) {
					throw std::format_error("fmtout: unmatched {");
				}
				if (fmt[i + 1] == '{') {
					ss << '{';
					++i;
					...
					if (fmt[i] == '}') {
						...
						if (auto_indexing) {
							...
							throw std::format_error("fmtout: cannot mix manual and automatic indexing");
						}
					}
					else {
						if (auto_indexing) {
						...
						for (; fmt[i] != '}'; ++i) {
							...
							if (fmt[i] < '0' || fmt[i] > '9') {
								throw std::format_error(std::string("fmtout: expected a digit but got ") + fmt[i]);
							}
							index_str += fmt[i];
						}
